...
migrate((app) => {
  ...
  for (const user of allContractors) {
    ...
    const profession = user.getString("profession") || "General contractor";

    const record = new Record(contractorsCol);
    record.set("userId", user.id);
    record.set("name", user.getString("name") || user.getString("email"));
    record.set("profession", profession);
    record.set("location", user.getString("location") || "Lithuania");
    ...
    try {
      app.save(record);
      console.log(`Created contractor record for: ${user.getString("email")}`);
    } catch (e) {
      console.log(`Failed for ${user.getString("email")}: ${e.message}`);
    }
  }
}, (app) => {});
