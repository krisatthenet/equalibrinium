  ...
  try {
    ...
    for (const search of searches) {
          ...
          if (accountSid && authToken && from) {
            ...
            try {
              $http.send({
                ...
                body: `From=${encodeURIComponent(from)}&To=${encodeURIComponent(n)}&Body=${encodeURIComponent('WorkBee: ' + contractorName + ' matches your saved search "' + label + '". View them at workbee.space/explore')}`,
              });
            } catch (_) {}
          }
        }
      ...
      try {
        ...
        if (secret) {
          $http.send({
            ...
            body: JSON.stringify({ userId: clientId, title: 'New match: ' + label, body: contractorName + ' just joined WorkBee', url: '/explore' }),
          });
        }
      } catch (_) {}
      ...
      try {
        ...
        rec.set('title',  'New contractor matches "' + label + '"');
