...
// Wrapped in IIFE so helper functions are captured via closure by the callback,
// avoiding Goja runtime scope issues with top-level function declarations.
(function() {
  function sendSms(to, body) {
    ...
    try {
      ...
      if (res.statusCode >= 400) $app.logger().error('SSA SMS error', 'status', res.statusCode);
    } catch (err) {
      $app.logger().error('SSA SMS send failed', 'error', err.message);
    }
  }

  function ssaSendPush(userId, title, body, url) {
  ...
  function createNotification(userId, type, title, body, link) {
    try {
      ...
      $app.logger().error('SSA createNotification failed', 'error', err.message);
    }
  }
  ...
  onRecordAfterCreateSuccess((e) => {
    ...
    try {
      ...
      for (const search of searches) {
        ...
        ssaSendPush(clientId, `New match: ${label}`, `${contractorName} just joined WorkBee`, '/explore');
        createNotification(clientId, 'search_alert', `New contractor matches "${label}"`, `${contractorName} is now available.`, '/explore');

        $app.logger().info('SSA alert sent', 'clientId', clientId, 'contractor', contractorName, 'label', label);
      }
    } catch (err) {
